...
with st.container(key=f"scale_box_{st.session_state.step}"):
    ...
    if st.session_state.step == 0:
        ...
        st.markdown('<div class="slide-title">ОБІД І ГУРТКИ</div>', unsafe_allow_html=True)
        st.markdown('<div style="font-size:18px;color:#ddd;margin-bottom:20px;">🍽 14:00. Після обіду — піонерські справи.</div>', unsafe_allow_html=True)
        cl, cr = st.columns([2,1], gap="large")
        with cl:
            st.markdown('<div class="lesson-card"><div class="lesson-title">📌 Піонерські справи</div><div class="lesson-text">• Збори загону<br>• Збір макулатури<br>• Допомога старшим<br>• Спортивні секції<br>• Творчі гуртки</div></div>', unsafe_allow_html=True)
        with cr:
        ...
        with c1:
            if st.button("⬅ Назад", key="back_btn_6"):
                ...
                st.rerun()
        with c2:
            if st.button("Далі ➔", key="next_btn_6"):
        ...
        st.markdown('<div class="slide-title">МОДА ТА КУЛЬТУРА</div>', unsafe_allow_html=True)
        st.markdown(
            '<div class="lesson-card" style="margin-bottom:14px;"><div class="lesson-title">🎸 Музика</div><div class="lesson-text">The Beatles, The Rolling Stones. Радянська естрада: Магомаєв, П\'єха.</div></div>'
            '<div class="lesson-card" style="margin-bottom:14px;"><div class="lesson-title">🎬 Кіно</div><div class="lesson-text">«Людина-амфібія», «Гусарська балада», «Кавказька полонянка», «Діамантова рука».</div></div>'
            '<div class="lesson-card"><div class="lesson-title">🎲 Ігри</div><div class="lesson-text">Класики, гумовий стрибок, футбол у дворі, шахи, шашки.</div></div>',
            unsafe_allow_html=True
        )
        st.write("")
        c1, c2, _ = st.columns([1,1,4])
        ...
        with c1:
            if st.button("⬅ Назад", key="back_btn_11"):
                ...
                st.rerun()
        with c2:
            if st.button("Далі ➔", key="next_btn_11"):
        ...
        choice = st.radio("Оберіть:", [
            ...
            "В) Побіжу до школи без сніданку",
            "Г) Полежу з телефоном"
        ], key="test1_choice", label_visibility="collapsed")
        st.write("")
        if st.button("Зробити вибір", key="test1_btn"):
            if choice.startswith("А"):
                st.success("✅ **07:15.** Ти бадьорий, поснідав кашею. **Ідеальний ранок піонера!**")
            elif choice.startswith("Б"):
                st.warning("⚠️ Не встиг поснідати. На уроці думаєш про їжу.")
        ...
        if st.button("Зробити вибір", key="test2_btn"):
            if choice.startswith("А"):
                ...
                st.error("❌ Друкарських машинок у школах не було.")
        st.write("")
        c1, c2, _ = st.columns([1,1,4])
        with c1:
        ...
        choice = st.radio("Оберіть:", [
            ...
            "В) Піду на футбол з друзями",
            "Г) Дивитимусь телевізор"
        ...
        if st.button("Зробити вибір", key="test3_btn"):
            if choice.startswith("А"):
                ...
                st.warning("⚠️ Телевізор був рідкістю.")
        st.write("")
        c1, c2, _ = st.columns([1,1,4])
        with c1:
        ...
        st.markdown('<div class="slide-title">ЩО В ТОРБІ ПІОНЕРА?</div>', unsafe_allow_html=True)
        st.markdown('<div style="font-size:16px;color:#ddd;margin-bottom:20px;">Познач предмети, які піонер 60-х носив із собою.</div>', unsafe_allow_html=True)
        items = [
