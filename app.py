...
.stButton > button:hover { background-color: #505050 !important; border-color: #d4af6a !important; }
.lesson-card { background-color: #3a3a3a; border-left: 6px solid #d4af6a; padding: 18px 20px; border-radius: 8px; width: 100%; }
...
div[data-testid="stImage"] img { max-height: 38vh !important; width: auto !important; max-width: 100% !important; object-fit: contain !important; border-radius: 12px !important; margin: 0 auto !important; display: block !important; }
...
with st.container(key=f"scale_box_{st.session_state.step}"):
    ...
    if st.session_state.step == 0:
        ...
        st.markdown(
            ...
            '<div style="font-size:18px;color:#ddd;line-height:1.7;margin-bottom:20px;">Піонери 60-х та сучасна молодь жили в різних світах. Одні виховувались у колективі, де головним було спільне благо та дисципліна.</div>'
            '<div style="font-size:18px;color:#ddd;line-height:1.7;">Інші — у світі, де цінується свобода вибору. Давайте порівняємо.</div>',
            unsafe_allow_html=True
        )
        st.write("")
        c1, c2, _ = st.columns([1,1,4])
        ...
        st.markdown(
            ...
            '<div class="question-card"><b>Ситуація:</b> Дзвенить будильник о 07:00. Мама вже на кухні, по радіо грає мелодія для зарядки.<br><br><b>Питання:</b> Що робитимеш?</div>',
            unsafe_allow_html=True
        )
        ...
        ], key="test1_choice", label_visibility="collapsed")
        st.write("")
        if st.button("Зробити вибір", key="test1_btn"):
            if choice.startswith("А"):
                st.success("✅ **07:15.** Ти бадьорий, зробив зарядку, поснідав кашею. **Ідеальний ранок піонера!**")
            elif choice.startswith("Б"):
                st.warning("⚠️ Схопився з ліжка, не встиг поснідати. На уроці думаєш про їжу.")
            elif choice.startswith("В"):
                st.error("❌ Вибіг голодним. На третій годині живіт бурчить на весь клас.")
        ...
        st.markdown(
            ...
            '<div class="question-card"><b>Ситуація:</b> Учитель дав контрольну роботу. Треба написати твір.<br><br><b>Питання:</b> Чим будеш писати?</div>',
            unsafe_allow_html=True
        )
        choice = st.radio("Оберіть:", [
            "А) Кульковою ручкою",
        ...
        ], key="test2_choice", label_visibility="collapsed")
        st.write("")
        if st.button("Зробити вибір", key="test2_btn"):
            if choice.startswith("А"):
                st.error("❌ У 60-х кулькові ручки були рідкістю.")
            elif choice.startswith("Б"):
        ...
        st.markdown(
            ...
            '<div class="question-card"><b>Ситуація:</b> Уроки закінчились о 14:00. Попереду — вільний час.<br><br><b>Питання:</b> Що робитимеш?</div>',
            unsafe_allow_html=True
        )
        choice = st.radio("Оберіть:", [
        ...
        ], key="test3_choice", label_visibility="collapsed")
        st.write("")
        if st.button("Зробити вибір", key="test3_btn"):
            if choice.startswith("А"):
                st.success("✅ **Молодець!** Ти справжній піонер!")
            elif choice.startswith("Б"):
                st.error("❌ У 1960-х комп'ютерів вдома не було!")
            elif choice.startswith("В"):
                st.success("✅ Добре! Спорт — теж піонерська справа.")
            elif choice.startswith("Г"):
